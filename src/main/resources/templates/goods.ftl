        ...
        <div class="layui-col-xs8" style="padding: 10px">
            <ul class="bb-info">
                ...
                <li style="color: darkgrey">
                    <span>宝宝团价：</span><span style="font-size: 26px;color: orange;font-weight: bold">￥${goods.currentPrice}</span><span
                        style="text-decoration: line-through">￥${goods.originalCost}</span>
                </li>

                <li>
                    <span><#if goods.isFreeDelivery == 1 >满58.0元包邮</#if></span>
                </li>
                <li>
                    数量：
    ...
    <div class="layui-row">
        <div class="layui-tab layui-tab-brief" lay-filter="docDemoTabBrief">
            ...
            <div class="layui-tab-content">
                ...
                <div id="divEvaluate" class="layui-tab-item layui-col-xs8 layui-col-xs-offset2">
                    <h3>好用哦</h3><br>
                </div>
            </div>
        </div>
...
<script>
    ...
    ;!function () {
        var layer = layui.layer
            , form = layui.form;
        var $ = layui.$; //$是jquery的别名，layui内置了jquery
        $.getJSON("/evaluate/${goods.goodsId?string('0')}", function (json){
            for(var i=0; i < json.length; i++){
                var eva = json[i];
                $("#divEvaluate").append("<h3>"+eva.createTime + "宝妈评价道"+ eva.content + "</h3><br>")
            }
            console.info(json)
        })
